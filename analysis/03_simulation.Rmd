...
plan(multisession)
source("code/ras_ss.R") # shared scan/detect helper functions
```
...
# save after each scenario so a crash doesn't lose the whole run.
# v2 stores raw FN counts; the type-I file is the older one, leave it alone
...
res$ss_power <- res$ss_fn == 0   # caught every true region
...
```{r bench, cache=TRUE}
# per-rep runtime, single core. Cohort simulation is paid once up front and
# excluded from ras-ss/indiv so the comparison isolates the scan step.
nb <- 30
bench <- function(f) mean(sapply(1:nb, function(s) system.time(f(s))["elapsed"]))
bb <- make_beta(3, 0.5, c_eff$continuous, 7)

cohorts <- lapply(1:nb, function(s) sim_cohorts(bb, "continuous", 900000+s))
grid_x <- seq(1, n_snps, by = skip1)

t_ss <- bench(function(s) { co <- cohorts[[s]]
  scan <- scan_ss(co$disc$beta, co$targ$z, R_emp, prune_filter)
  invisible(run_detect(scan, sl=slope_thresh, sr=slope_thresh, dav=davies_thresh)) })
t_in <- bench(function(s) { co <- cohorts[[s]]
  prof <- indiv_scan(list(x=grid_x), co$X_t, co$y_t, co$disc$beta)
  invisible(run_detect(list(x=grid_x, y=prof), sl=slope_thresh, sr=slope_thresh, dav=davies_thresh)) })
t_pu <- bench(function(s) { pp <- quiet(native_run_via_ras(bb, 920000+s))
  invisible(run_detect(pp, sl=slope_thresh, sr=slope_thresh, dav=davies_thresh)) })
```
...
knitr::kable(t1_tab, caption="Type I error at the calibrated thresholds (exact 95% CI), 200 null reps per trait")

knitr::kable(data.frame(method=c("ras-ss","indiv","pure ras()"), s_per_rep=round(c(t_ss,t_in,t_pu),1)),
             caption="Wall-clock seconds per rep, single core")

# per-region detection rate is the readable metric here, since the CPD
# usually locks onto 1-2 of the 3 peaks on these saturated synthetic profiles
...
# ras-ss vs the individual-level run on the same data; on the dashed line = they agree
...
# the paper's own definition: detected every true region
